...
&НаКлиенте
Перем ИдентификаторКлиента, АдресПриложения;

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	ИдДок = "dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6YTM2MHZpZXdlci90MTYwNDczODY3MjI1N18wMTI5Mzc2MTY0MzM3Njc0NTJfMTYwNDczODY3MjI3Mi5ydnQ";
...
Процедура ЗагрузитьТокенБраузера(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		АдресURL = АдресПриложения + "?Token=" + Токен + "&DocId=urn:" + ИдДок;
...
Процедура ОткрытьAutodesk(Команда)
	...
	АдресURL = АдресПриложения + "?Token=" + Токен + "&DocId=" + ИдДок;
	HTTPЗапрос = Новый HTTPЗапрос("/json/new?" + АдресURL);
	HTTPСоединение = Новый HTTPСоединение("localhost", ПортБраузера, , , , 10);
	Попытка
		HTTPОтвет = HTTPСоединение.Получить(HTTPЗапрос);
...
Процедура МодельПриИзменении(Элемент)
	...
	ТекстСкрипта = "{
		|viewer.loadDocumentNode(md_ViewerDocument, md_viewables[" + Формат(Модель, "ЧГ=") + "]);
		|}";
	ВыполнитьJavaSctipt(ТекстСкрипта, Новый ОписаниеОповещения);
